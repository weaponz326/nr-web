...
import { Rink } from 'projects/shop/src/app/models/modules/portal/portal.model';
import { AccountApiService } from 'projects/shop/src/app/services/account-api/account-api.service';
import { PortalApiService } from 'projects/shop/src/app/services/modules-api/portal-api/portal-api.service';
...
export class NewRinkComponent implements OnInit {
  ...
  getRecipientDetail(){
    this.accountApi.getSearchDetail(String(sessionStorage.getItem('shopSearchAccount')))
      .subscribe({
        next: (res) => {
          console.log(res);
  ...
  createRink(){
    let data: Rink = {
      ...
      sender: this.customCookie.getCookie('shop_id') as string,
      recipient: sessionStorage.getItem('shop_rink_recipient') as string,
    }
    ...
    this.portalApi.postRink(data)
      .subscribe({
        next: (res) => {
          ...
          sessionStorage.setItem('shop_rink_id', res.id);
