...
export class NewRinkComponent implements OnInit {
  ...
  getRecipientDetail(){
    this.accountApi.getSearchDetail(String(sessionStorage.getItem('schoolSearchAccount')))
      .subscribe({
        next: (res) => {
          console.log(res);
  ...
  createRink(){
    let data: Rink = {
      ...
      sender: localStorage.getItem('school_id') as string,
      recipient: sessionStorage.getItem('school_rink_recipient') as string,
    }
