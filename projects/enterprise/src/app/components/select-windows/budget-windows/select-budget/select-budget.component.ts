...
export class SelectBudgetComponent implements OnInit {
  ...
  openModal(){
    this.budgetGridData = [];
    this.getAccountBudgets(1, 20, "-created_at");
    this.openButton.nativeElement.click();
  }

  getAccountBudgets(page: any, size: any, sortField: any){
    this.isFetchingGridData = true;

    this.budgetApi.getAccountBudgets(page, size, sortField)
      .subscribe({
        next: (res) => {
          console.log(res);
  ...
  sortTable(column: any){
    console.log(column);
    this.getAccountBudgets(1, 20, column);
