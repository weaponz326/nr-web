...
@Component({
  selector: 'app-select-sent-letter',
  templateUrl: './select-sent-letter.component.html',
  styleUrls: ['./select-sent-letter.component.scss']
})
export class SelectSentLetterComponent implements OnInit {
  ...
  getAccountSentLetters(page: any, size: any, sortField: any){
    ...
    this.lettersApi.getAccountSentLetters(page, size, sortField)
      .subscribe({
        next: (res) => {
          console.log(res);
          this.sentLettersGridData = res.results;
