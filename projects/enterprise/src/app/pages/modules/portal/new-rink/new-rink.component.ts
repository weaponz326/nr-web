...
import { Rink } from 'projects/enterprise/src/app/models/modules/portal/portal.model';
...
export class NewRinkComponent implements OnInit {
  ...
  getRecipientDetail(){
    this.accountApi.getSearchDetail(String(sessionStorage.getItem('enterpriseSearchAccount')))
      .subscribe({
        next: (res) => {
          console.log(res);
  ...
  createRink(){
    let data: Rink = {
      ...
      sender: this.customCookie.getCookie('enterprise_id') as string,
      recipient: sessionStorage.getItem('enterprise_rink_recipient') as string,
    }
    ...
    this.portalApi.postRink(data)
      .subscribe({
        next: (res) => {
          ...
          sessionStorage.setItem('enterprise_rink_id', res.id);
