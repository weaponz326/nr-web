...
export class SearchViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(sessionStorage.getItem('hotelSearchInput')){
      ...
      this.searchFilter = String(sessionStorage.getItem('hotelSearchFilter') || 'Personal');
  ...
  getSearchDetail(userId: any){
    sessionStorage.setItem('hotelSearchAccount', userId);

    this.accountApi.getSearchDetail(sessionStorage.getItem('hotelSearchAccount') as string)
      .subscribe({
        next: (res) => {
          console.log(res);
