...
import { Invitation } from 'projects/hotel/src/app/models/modules/admin/admin.model';
...
export class UserSearchComponent implements OnInit {
  ...
  ngOnInit(): void {
    console.log(sessionStorage.getItem('hotelAdminSearchInput'));

    if(sessionStorage.getItem('hotelAdminSearchInput')){
      this.searchQuery = sessionStorage.getItem('hotelAdminSearchInput');
      this.doSearch(1);
    }
  }
  ...
  doSearch(page: any){
    if (this.searchInput.trim() != ""){
      ...
      sessionStorage.setItem('hotelAdminSearchInput', this.searchInput);
  ...
  getSearchDetail(){
    this.authApi.getSearchDetail(String(sessionStorage.getItem('hotelAdminSearchUser')))
      .subscribe({
        next: (res) => {
          console.log(res);
  ...
  gotoSearchDetail(userId: any){
    sessionStorage.setItem('hotelAdminSearchUser', userId);
