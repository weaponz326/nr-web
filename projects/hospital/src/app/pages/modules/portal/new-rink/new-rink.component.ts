...
import { Rink } from 'projects/hospital/src/app/models/modules/portal/portal.model';
import { AccountApiService } from 'projects/hospital/src/app/services/account-api/account-api.service';
import { PortalApiService } from 'projects/hospital/src/app/services/modules-api/portal-api/portal-api.service';
...
export class NewRinkComponent implements OnInit {
  ...
  getRecipientDetail(){
    this.accountApi.getSearchDetail(String(sessionStorage.getItem('hospitalSearchAccount')))
      .subscribe({
        next: (res) => {
          console.log(res);
  ...
  createRink(){
    let data: Rink = {
      ...
      sender: this.customCookie.getCookie('hospital_id') as string,
      recipient: sessionStorage.getItem('hospital_rink_recipient') as string,
    }
    ...
    this.portalApi.postRink(data)
      .subscribe({
        next: (res) => {
          ...
          sessionStorage.setItem('hospital_rink_id', res.id);
