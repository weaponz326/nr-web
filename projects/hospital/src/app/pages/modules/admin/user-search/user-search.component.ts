...
import { Invitation } from 'projects/hospital/src/app/models/modules/admin/admin.model';
...
export class UserSearchComponent implements OnInit {
  ...
  ngOnInit(): void {
    console.log(sessionStorage.getItem('hospitalAdminSearchInput'));

    if(sessionStorage.getItem('hospitalAdminSearchInput')){
      this.searchQuery = sessionStorage.getItem('hospitalAdminSearchInput');
      this.doSearch(1);
    }
  }
  ...
  doSearch(page: any){
    if (this.searchInput.trim() != ""){
      ...
      sessionStorage.setItem('hospitalAdminSearchInput', this.searchInput);
  ...
  getSearchDetail(){
    this.authApi.getSearchDetail(String(sessionStorage.getItem('hospitalAdminSearchUser')))
      .subscribe({
        next: (res) => {
          console.log(res);
  ...
  gotoSearchDetail(userId: any){
    sessionStorage.setItem('hospitalAdminSearchUser', userId);
